...
class Network:
    ...
    def predict(self, item):
        ...
        for layer_weights in self.weights[:]:
            data_temp = np.maximum(0, (np.dot(layer_weights, data_temp)))
        return data_temp
...
def population_init(population_size, input_size, output_size):
    ...
    for _ in range(population_size):
        struct = [input_size, HIDDEN1, HIDDEN2, HIDDEN3, output_size]  # Fixed network structure
...
def evolve(population, train_data):
    ...
    for _ in range(10):
        structure = [INPUT_SIZE, HIDDEN1, HIDDEN2, HIDDEN3, OUTPUT_SIZE]  # Fixed network structure
...
train_data = read_file("train0.txt")  # Split the data into training and test sets
test_data = read_file("test0.txt")
...
with open("wnet0.json", "w") as json_file:
